...
int main(int argc, char* argv[])
{
 ...
 ProgramSettings Program_Settings {false,false,"","","",true};
  ...
  size_t caesar_key {0};
  if ( ! Program_Settings.cipher_key.empty() ) {
    ...
    for ( const auto& elem : Program_Settings.cipher_key ) {
      if ( ! std::isdigit(elem) ) {
	std::cerr << "[error] cipher key must be an unsigned long integer for Caesar cipher,\n"
	          << "        the supplied key (" << Program_Settings.cipher_key << ") could not be successfully converted" << std::endl;
	return 1;
      }
    }
    caesar_key = std::stoul(Program_Settings.cipher_key);
  }
